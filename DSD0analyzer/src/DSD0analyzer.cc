...
void DSD0Analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  cout << slowPiTracks.size() << " " << goodTracks.size() << endl;
      ...
      for(size_t k=j+1;k<goodTracks.size();k++){
        ...
        for(size_t l=k+1;l<goodTracks.size();l++){
          ...
          for(size_t s=0;s<slowPiTracks.size();s++){
            ...
            if( fabs(md0-1.86484)  > 1.) continue; 
            ...
            double dsmass = fabs(d0mass1-1.86484)<fabs(d0mass2-1.86484) ? dsmass1 : dsmass2;
            if( (dsmass - d0mass) > 0.16) continue;
            ...
            K3piTrkScharge.push_back(trkS->charge());

            NK3piCand++; 


            // some events were exhausting the memory per process saving 700MB of crap for event..
...
tree2->Branch("K3piTrkScharge",&K3piTrkScharge);
//MC
